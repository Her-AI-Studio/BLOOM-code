...
ui.on_message('photo_saved', message => {
  ...
  showStatus(`Photo saved as entry ${message.catalog_no || '#' + message.id}. Sketch it, then ask the local model about it below.`);
...
ui.on_message('note_updated', message => {
  if (currentDetailEntry && currentDetailEntry.id === message.id) {
    ...
    currentDetailEntry.locality = message.locality;
    currentDetailEntry.weather = message.weather;
    currentDetailEntry.habitat = message.habitat;
    ...
    const fieldsViewEl = document.getElementById('detailFieldsView');
    if (fieldsViewEl) {
      fieldsViewEl.innerHTML = `
        <p><strong>Locality:</strong> ${message.locality ? escapeHtml(message.locality) : '—'}</p>
        <p><strong>Weather:</strong> ${message.weather ? escapeHtml(message.weather) : '—'}</p>
        <p><strong>Habitat:</strong> ${message.habitat ? escapeHtml(message.habitat) : '—'}</p>
      `;
    }
...
function triggerCapture() {
  // Grinnell-method field conditions -- these persist across captures
  // (not cleared after each one) since weather/locality/habitat usually
  // stay the same for a whole outing, not just one entry.
  ...
  const habitat = document.getElementById('habitatInput')?.value.trim() || '';
  ui.send_message('capture', { locality, weather, habitat });
  ...
  list.innerHTML = entries.map(e => `
    <div class="entry-row" data-id="${e.id}">
      ...
      <div class="entry-meta">
        <div class="entry-time">${e.catalog_no ? escapeHtml(e.catalog_no) + ' · ' : ''}${formatTimestamp(e.timestamp)}${e.locality ? ' — ' + escapeHtml(e.locality) : ''}</div>
...
function renderEntryDetail(entry) {
  ...
  content.innerHTML = `
    <div class="catalog-tag">${escapeHtml(entry.catalog_no || ('#' + entry.id))}</div>
    ...
    <div id="detailFieldsView" class="field-conditions-view">
      <p><strong>Locality:</strong> ${entry.locality ? escapeHtml(entry.locality) : '—'}</p>
      <p><strong>Weather:</strong> ${entry.weather ? escapeHtml(entry.weather) : '—'}</p>
      <p><strong>Habitat:</strong> ${entry.habitat ? escapeHtml(entry.habitat) : '—'}</p>
    </div>
    <div id="detailFieldsEdit" class="field-conditions" style="display: none">
      <div class="field-conditions-row">
        <label for="detailLocalityInput">Locality</label>
        <input id="detailLocalityInput" type="text" value="${entry.locality ? escapeHtml(entry.locality) : ''}" />
      </div>
      <div class="field-conditions-row">
        <label for="detailWeatherInput">Weather</label>
        <input id="detailWeatherInput" type="text" value="${entry.weather ? escapeHtml(entry.weather) : ''}" />
      </div>
      <div class="field-conditions-row">
        <label for="detailHabitatInput">Habitat</label>
        <input id="detailHabitatInput" type="text" value="${entry.habitat ? escapeHtml(entry.habitat) : ''}" />
      </div>
    </div>
...
function attachDetailActionListeners(entry) {
  ...
  const syncStatus = document.getElementById('detailSyncStatus');
  const fieldsViewEl = document.getElementById('detailFieldsView');
  const fieldsEditEl = document.getElementById('detailFieldsEdit');
  const localityInput = document.getElementById('detailLocalityInput');
  const weatherInput = document.getElementById('detailWeatherInput');
  const habitatInput = document.getElementById('detailHabitatInput');

  if (editBtn) {
    editBtn.addEventListener('click', () => {
      viewEl.style.display = 'none';
      editEl.style.display = 'block';
      if (fieldsViewEl) fieldsViewEl.style.display = 'none';
      if (fieldsEditEl) fieldsEditEl.style.display = 'block';
  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', () => {
      ...
      const habitat = habitatInput ? habitatInput.value.trim() : '';
      ui.send_message('update_note', { entry_id: entry.id, text, locality, weather, habitat });
      viewEl.style.display = 'block';
      editEl.style.display = 'none';
      if (fieldsViewEl) fieldsViewEl.style.display = 'block';
      if (fieldsEditEl) fieldsEditEl.style.display = 'none';
